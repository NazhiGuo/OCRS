...
class GCN(Module):
    def __init__(self, user_feature, item_feature, rating):
        ...
        self.rating = rating
        self.num_user = rating['stu_id'].max() + 1
        self.num_item = rating['course_index'].max() + 1

        # user embedding
        self.user_id_embedding = nn.Embedding(user_feature['stu_id'].max() + 1, 64)
        # self.user_age_embedding = nn.Embedding(user_feature['age'].max() + 1, 4)
        # self.user_gender_embedding = nn.Embedding(user_feature['gender'].max() + 1, 32)
        ...
        # item embedding
        self.item_id_embedding = nn.Embedding(item_feature['course_index'].max() + 1, 32)
        self.item_type_embedding = nn.Embedding(item_feature['type_id'].max() + 1, 32)
        ...
        for _ in range(self.gcn_layers):
            self.GCN_Layers.append(GCN_Layer(self.embedSize, self.embedSize))
        self.graph = self.buildGraph()
        self.transForm = nn.Linear(in_features=self.embedSize * (self.gcn_layers + 1),
                                   out_features=self.embedSize)
    ...
    def getFeature(self):
        ...
        user_id = self.user_id_embedding(torch.tensor(self.user_feature['stu_id']).to(self.device))
        ...
        item_id = self.item_id_embedding(torch.tensor(self.item_feature['course_index']).to(self.device))
        item_type = self.item_type_embedding(torch.tensor(self.item_feature['type_id']).to(self.device))
        ...
        item_emb = torch.cat((item_id, item_type), dim=1)
        # 拼接到一起
