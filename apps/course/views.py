...
class CourseRec(View):
    '''课程列表'''
    def get(self, request):
        if request.user.is_authenticated:
            cousrec = tx.predict(request.user.id-1)
            all_courses = Course.objects.filter(id__in=cousrec)
        else:
        ...
        if int(page)>5:
            page=1
        courses = p.page(page)
        return render(request, "course-recommder.html", {
            "all_courses":courses,
            'sort': sort,
            'hot_courses':hot_courses,
